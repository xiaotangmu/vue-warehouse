<template>
  <div class="out-manager-div">
    <div class="main-box-view-title">
      <span class="main-box-view-title-name">出库管理 <span class="glyphicon glyphicon-question-sign"></span></span>
    </div>

    <el-tabs v-model="outMenu" style="margin-left: 20px;">
      <el-tab-pane label="出库维护" name="first"></el-tab-pane>
      <el-tab-pane label="出库添加" name="second"></el-tab-pane>
    </el-tabs>
                ...
                <template slot-scope="props">
                  <el-form label-position="left" inline class="demo-table-expand">
                    <el-form-item label="出库单号" style="margin-right: 40px">
                      <el-tag effect="plain"><span>{{ props.row.outNum }}</span></el-tag>
    ...
    <div v-if="outMenu === 'second'" style="margin-left: 20px">
      ...
      <div class="out_item_div">
        <el-tag
          ...
          type="info"
        >客户</el-tag>
        <el-select clearable v-model="clientId" placeholder="张老板" style="width: 200px">
          <el-option
            v-for="item in clients"
            :key="item.id"
            :label="item.name"
  ...
  export default {
    ...
    methods: {
      ...
      updateDesc(){
        ...
        axios.post('api/out/description', qs.stringify(data)).then(
          response => {
      ...
      checkEntryByCondition(pageNum, pageSize,type){
        const { warehouseId2, clientId2, outDate2, outNum , conditionCheckData} = this;

        const url = 'api/out/checkByCondition';
        let data = '1';
        if(type === 1){

          if(warehouseId2 === '' && clientId2 === '' && outDate2 === '' ){
            this.$message.warning('请先添加查询条件');
            return;
          }

          if(outDate2 === ''){
            data = { warehouseId: warehouseId2, clientId: clientId2, outDate: ''
              , outNum: '', pageNum: pageNum, pageSize: pageSize };
          }else{
            data = { warehouseId: warehouseId2, clientId: clientId2, outDate: outDate2.toLocaleDateString()
              , outNum: '', pageNum: pageNum, pageSize: pageSize };
          }
          ...
          data = { warehouseId: warehouseId2, clientId: clientId2, outDate: ''
            , outNum: outNum, pageNum: pageNum, pageSize: pageSize };

        }else {
          data = { warehouseId: conditionCheckData.warehouseId, clientId: conditionCheckData.clientId, outDate: conditionCheckData.outDate
            , outNum: conditionCheckData.outNum, pageNum: pageNum, pageSize: pageSize };
        }
      ...
      //提交出库单
      addEntry(){
        const { batch, clientId, clients, warehouseId, warehouses, outDate, operator, productList, totalPrice } = this;
        ...
        if(outDate === null || outDate === undefined || outDate === ''){
        ...
        const data = { batch: batch, clientId: client.id, clientName: client.name,warehouseId: warehouse.id,warehouseName: warehouse.name, outDate: outDate.toLocaleDateString(), operator: operator
          , productList: productList, totalPrice: totalPrice};

        const data2 = { outStr: JSON.stringify(data)};
        const url = 'api/out/add';
...
<style>
  ...
  .out-manager-list .td-1-sort,
  .out-manager-list .td-2-outbox{
    width: 70px;
  }
  .out-manager-head-btn-group .form-inline{
    margin-left: 290px;
    /*float: right;*/
